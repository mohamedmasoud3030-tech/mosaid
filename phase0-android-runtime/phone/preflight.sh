...
REPORT_DIR="$P0_HOME/reports"
NETWORK=0
[[ "${1:-}" == "--network" ]] && NETWORK=1
mkdir -p "$REPORT_DIR" "$P0_HOME/tmp"
REPORT="$REPORT_DIR/preflight-$(date -u +%Y%m%dT%H%M%SZ).json"
TMP="$P0_HOME/tmp/preflight.$$"
mkdir -p "$TMP"
trap 'rm -rf "$TMP"' EXIT
...
storage_kb="$(df -Pk "$P0_HOME" | awk 'NR==2{print $4}')"
mem_total_kb="$(awk '/^MemTotal:/{print $2}' /proc/meminfo 2>/dev/null || echo 0)"
mem_available_kb="$(awk '/^MemAvailable:/{print $2}' /proc/meminfo 2>/dev/null || echo 0)"

write_ok=false
probe="$P0_HOME/tmp/write-probe.$$"
if (umask 077; printf 'phase0\n' > "$probe") 2>/dev/null && [[ "$(cat "$probe")" == phase0 ]]; then write_ok=true; fi
rm -f "$probe"
...
if (( NETWORK )); then
  ...
  api_base="$(jq -r '.model_list[0].api_base' "$CONFIG")"
  model_id="$(jq -r '.model_list[0].model | sub("^openai/"; "")' "$CONFIG")"
  jq -nc --arg m "$model_id" '{model:$m,messages:[{role:"user",content:"Reply exactly PHASE0_MODEL_OK"}],max_tokens:16,temperature:0}' > "$TMP/model-request.json"
  ...
  if [[ "$model_text" == *PHASE0_MODEL_OK* ]]; then model_api="pass"; else model_api="fail"; fi
fi
