...
# Mosaid product supervisor: singleton lock, wake lock, restart backoff,
# log redaction, health sampling and clean shutdown.
set -euo pipefail
umask 077
...
BIN="$MOSAID_HOME/bin/mosaid"
SCRIPTS="$MOSAID_HOME/scripts"
RUNTIME="$MOSAID_HOME/runtime"
CONFIG="$MOSAID_HOME/config.json"
mkdir -p "$RUNTIME" "$MOSAID_HOME/logs" "$MOSAID_HOME/tmp"
...
while :; do
  ...
  MOSAID_HOME="$MOSAID_HOME" "$BIN" -config "$CONFIG" > "$fifo" 2>&1 &
  child_pid=$!
