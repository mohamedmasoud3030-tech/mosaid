...
# Sample Mosaid product health every PHASE14_HEALTH_INTERVAL_SECONDS
# (default 60). Always refreshes runtime/health.json; when tests/active.json
# declares a running scenario, samples are also appended to that scenario's
# samples.csv until its end_epoch passes.
set -euo pipefail
umask 077
...
RUNTIME="$MOSAID_HOME/runtime"
TESTS="$MOSAID_HOME/tests"
INTERVAL="${PHASE14_HEALTH_INTERVAL_SECONDS:-60}"
...
while kill -0 "$child_pid" 2>/dev/null; do
  ...
  disk_free_kb="$(df -Pk "$MOSAID_HOME" | awk 'NR==2{print $4}')"
